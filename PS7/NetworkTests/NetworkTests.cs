...
namespace NetworkUtil
{
    ...
    public class NetworkTests
    {
        ...
        public void TestStopServer()
        {
            ...
            TcpListener listener = Networking.StartServer(saveClientState, 8675);
        ...
        [TestMethod]
        public void TestIPAdressTimeout()
        {
            ...
            TcpListener listener = Networking.StartServer(saveClientState, 9000);

            Networking.ConnectToServer(saveClientState, "193.168.0.55", 2000);
            NetworkTestHelper.WaitForOrTimeout(() => isCalled, NetworkTestHelper.timeout);

            Assert.IsTrue(isCalled);
            Assert.IsTrue(testLocalSocketState?.ErrorOccurred);
            Assert.AreEqual("Connection Timeout", testLocalSocketState?.ErrorMessage);

        }

        [TestMethod]
        public void TestInvalidIPAdress()
        {
            ...
            TcpListener listener = Networking.StartServer(saveClientState, 9001);

            Networking.ConnectToServer(saveClientState, "192.168.0.2@", 2001);
            NetworkTestHelper.WaitForOrTimeout(() => isCalled, NetworkTestHelper.timeout);

            Assert.IsTrue(isCalled);
            Assert.IsTrue(testLocalSocketState?.ErrorOccurred);
            Assert.AreEqual("Invalid IP Address", testLocalSocketState?.ErrorMessage);
        ...
        public void TestSendAndClose(bool clientSide)
        {
            ...
            Assert.IsFalse(Networking.Send(testLocalSocketState.TheSocket, "b"));

            Assert.IsFalse(Networking.SendAndClose(testLocalSocketState.TheSocket, "b"));
        ...
        [TestMethod]
        public void TestExceptionCatchGetData()
        {
            ...
            Assert.AreEqual("Error During Receive Process", testRemoteSocketState?.ErrorMessage);
