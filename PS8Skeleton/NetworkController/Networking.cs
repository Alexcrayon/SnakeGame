...
/// <summary>
/// Static class used to make the Networking of future assignments easier.
/// Contains methods server side and client side use to communicate.
/// Implemented by Lucas Jones and Alex Cao 11/11 2022
/// </summary>
public static class Networking
{
    ...
    {
        //Pass on the action delegate and listener using tuple 
        ...
        {
            SocketState error = new SocketState(toCall, "Error During Connection Process");
    ...
    public static void ConnectToServer(Action<SocketState> toCall, string hostName, int port)
    {
        ...
        try
        {
            ...
            if (!foundIPV4)
            {
                SocketState error = new SocketState(toCall, "Couldn't Find Any IPV4 Address");
                error.OnNetworkAction(error);
            }
        }
        catch (Exception)
        {
            ...
            {
                SocketState error = new SocketState(toCall, "Invalid IP Address");
                error.OnNetworkAction(error);
            }
        }
        ...
        //Timeout after 3 second of BeginConnect and produce a error message
        if (!result.AsyncWaitHandle.WaitOne(3000, true))
        {
            SocketState error = new SocketState(toCall, "Connection Timeout");
            error.OnNetworkAction(error);
        }
    }
    ...
    {
        // Use the passed down socketState
        ...
        {
            SocketState error = new SocketState(state.OnNetworkAction, "Connection Failed");
            error.OnNetworkAction(error);
        }
    }
    ...
    private static void ReceiveCallback(IAsyncResult ar)
    {
        ...
        try
        {
            ...
            //Detect if socket is closed
            if (numBytes == 0)
                throw new Exception("Socket Removed");
        }
        catch (Exception)
        {
            state.ErrorOccurred = true;
            state.ErrorMessage = "Error During End Receive Process";
        }

        lock (state.data)
        {
            //Read received data and use user's delegate to handld it
    ...
    {
        // if socket is closed, do not send
        if (!socket.Connected)
        {
            return false;
        }
        ...
        // if error occurs during send process, close the socket and return
        catch (Exception)
        {
            socket.Close();
            return false;
    ...
    private static void SendAndCloseCallback(IAsyncResult ar)
    {
        Socket socket = (Socket)ar.AsyncState!;
        socket.EndSend(ar);
        // shut down socket for send and receive, then close it
